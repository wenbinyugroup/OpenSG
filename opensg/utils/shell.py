...
def generate_boundary_markers(xmin, xmax):
    ...
    def is_left_boundary(x):
        return np.isclose(x[0], xmin,atol=0.0115)

    def is_right_boundary(x):
        return np.isclose(x[0], xmax,atol=0.0115)
...
def apply_null_A(A_l,nullspace_basis):
  ...
  #  for n in nullspace_basis:
    ...
    for pp in range(4):
        N.append(nullspace_basis[pp].array)
    ...
    NNT.setPreallocationNNZ(4)  # Each row has contributions from 6 RBMs
    NNT.setUp()
