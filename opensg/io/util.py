...
def generate_segment_shell_mesh_files(blade_mesh_yaml, segment_list=None, segment_folder=None):
    ...
    for segment_index in segment_list:
        ...
        node_in_segment = np.zeros(num_b_nodes, dtype=int) # Track which nodes are in the segment
        element_in_segment = np.zeros(num_b_elements, dtype=int) # Track which elements are in the segment
        element_layer_id = np.zeros(num_b_elements, dtype=int) # Track the layer number of each element
        ...
        segment_data = {
            ...
            'sets': {
                'element': [
                    {
                        'name': f'layup_{i}',
                        'labels': [j for j, subdomain in enumerate(segment_subdomains) if subdomain == i]
                    }
