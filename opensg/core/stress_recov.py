...
def recover_local_strain(timo, beam_force, segment, meshdata):
    ...
    fexpr1 = dolfinx.fem.Expression(
        st_3D, VV.element.interpolation_points(), comm=MPI.COMM_WORLD
    )
...
def local_stress(mat_param, segment_mesh, strain_3D, points):
    ...
    V_stiff = dolfinx.fem.functionspace(
        mesh, basix.ufl.element("DG", mesh.topology.cell_name(), 0, shape=(6, 6))
    )
    # UFL Stiffness
    ...
    V_stress = dolfinx.fem.functionspace(
        mesh, basix.ufl.element("CG", mesh.topology.cell_name(), 2, shape=(6,))
    )
...
def eigen_stiffness_matrix(mat_param, segment_mesh, strain_3D, N_eig):
    ...
    dx = ufl.Measure("dx")(
        domain=mesh, subdomain_data=segment_mesh.meshdata["subdomains"]
    )
    ...
    bcs = [
        dolfinx.fem.dirichletbc(
            ...
            dolfinx.fem.locate_dofs_topological(
                V, mesh.topology.dim - 1, segment_mesh.left_submesh["entity_map"]
            ),
            V,
        ),
        dolfinx.fem.dirichletbc(
            u_L,
            dolfinx.fem.locate_dofs_topological(
                V, mesh.topology.dim - 1, segment_mesh.right_submesh["entity_map"]
            ),
            V,
        ),
    ]  # Constrained both ends
    # Taking no contribution from end boundary

    # Linear Elasticity Bilinear Form
    a = sum(
        [
            ufl.dot(utils.sigma(du, i, CC)[1], utils.epsilon(u_)[1]) * dx(i)
    ...
    kgform = -sum(
        [
            ufl.inner(
                utils.sigma_prestress(i, CC, strain_3D)[0],
