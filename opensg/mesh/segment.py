...
class ShellSegmentMesh:
    ...
    def _build_boundary_submeshdata(self):
     ...
     #   opensg.utils.shared.xmdf_convert(left_mesh, left_entity_map)
      ...
      #  c_to_v = left_mesh.topology.connectivity(left_mesh.topology.dim, 0)  # cell_to_vertex connectivity
    ...
    #    l_parent_indices = l_map.sub_topology_to_topology(l_sub_indices, False)
        ...
        def _build_boundary_subdomains(boundary_meshdata):
            ...
            El1 = dolfinx.fem.Function(boundary_VV)
            El2= dolfinx.fem.Function(boundary_VV)
            Nl = dolfinx.fem.Function(boundary_VV)
            ...
            for i, xx in enumerate(boundary_entity_map):
                # assign subdomain
                pos = np.where(cell_of_facet_mesh.array == xx)[0][0]
                idx = int(np.searchsorted(cell_of_facet_mesh.offsets, pos, side='right') - 1)
                ...
                El2.x.array[3*i],El2.x.array[3*i+1],El2.x.array[3*i+2]=0,self.EE2.x.array[3*idx+1],self.EE2.x.array[3*idx+2]
                Nl.x.array[3*i],Nl.x.array[3*i+1],Nl.x.array[3*i+2] =0,self.N.x.array[3*idx+1],self.N.x.array[3*idx+2]
                # assign orientation
             #   for j in range(3):
...
class SolidSegmentMesh:
    ...
    #    self.taper_origin.append(float(mean) / blade_length)
        ...
        left_mesh, left_entity_map, left_vertex_map=(
            dolfinx.mesh.create_submesh(self.mesh, self.fdim, left_facets)
        )[0:3]

    #    opensg.utils.shared.xmdf_convert(left_mesh, left_entity_map)
        
        right_mesh, right_entity_map, right_vertex_map=(
            dolfinx.mesh.create_submesh(self.mesh, self.fdim, right_facets)
        )[0:3]
