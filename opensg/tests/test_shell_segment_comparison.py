...
class TestSegmentComparison(unittest.TestCase):
    ...
    def test_material_properties(self):
        ...
        self.assertEqual(set(blade_materials.keys()), set(standalone_materials.keys()),
        ...
        for material_name in blade_materials.keys():
            ...
            for prop in ['id', 'E', 'G', 'nu']:
                self.assertIn(prop, blade_mat, f"Blade material {material_name} missing {prop}")
                self.assertIn(prop, standalone_mat, f"Standalone material {material_name} missing {prop}")
                ...
                if isinstance(blade_val, (list, np.ndarray)):
                    self.assertTrue(np.allclose(blade_val, standalone_val, rtol=1e-10, atol=1e-10),
                                  f"Material {material_name} {prop} values should match")
                else:
                    self.assertEqual(blade_val, standalone_val,
                                   f"Material {material_name} {prop} values should match")
    ...
    def test_layup_properties(self):
        ...
        for key in required_keys:
            ...
            self.assertIn(key, standalone_layups, f"Standalone layup database missing {key}")
        ...
        self.assertEqual(blade_lengths, standalone_lengths,
        ...
        for key in required_keys:
            ...
            if key == 'mat_names':
                ...
                for i, (blade_item, standalone_item) in enumerate(zip(blade_array, standalone_array)):
                    self.assertEqual(blade_item, standalone_item,
                                  f"Layup {key}[{i}] should match")
            elif isinstance(blade_array[0], (list, np.ndarray)):
                # For nested arrays (like mat_names)
                for i, (blade_item, standalone_item) in enumerate(zip(blade_array, standalone_array)):
                    self.assertTrue(np.allclose(blade_item, standalone_item, rtol=1e-10, atol=1e-10),
                                  f"Layup {key}[{i}] should match")
            else:
                # For simple arrays (like thick, angle, nlay)
                self.assertTrue(np.allclose(blade_array, standalone_array, rtol=1e-10, atol=1e-10),
    ...
    def test_element_orientations(self):
        ...
        # Check lengths
        self.assertEqual(len(blade_orientations), len(standalone_orientations),
                        "Element orientations should have the same length")
        
        # Check each orientation
        for i, (blade_orient, standalone_orient) in enumerate(zip(blade_orientations, standalone_orientations)):
            self.assertTrue(np.allclose(blade_orient, standalone_orient, rtol=1e-10, atol=1e-10),
    ...
    def test_abd_matrices(self):
        ...
        # Check lengths
        self.assertEqual(len(blade_abd), len(standalone_abd),
                        "ABD matrices should have the same length")
        
        # Check each ABD matrix
        for i, (blade_matrix, standalone_matrix) in enumerate(zip(blade_abd, standalone_abd)):
            self.assertTrue(np.allclose(blade_matrix, standalone_matrix, rtol=1e-10, atol=1e-10),
    ...
    def test_stiffness_matrices(self):
        ...
        self.assertEqual(len(blade_stiffness), len(standalone_stiffness),
                        "Should return the same number of stiffness matrices")
        
        # Compare each stiffness matrix
        stiffness_names = ['Timoshenko', 'Euler-Bernoulli', 'Left boundary', 'Right boundary']
        for i, (name, blade_matrix, standalone_matrix) in enumerate(zip(stiffness_names, blade_stiffness, standalone_stiffness)):
            self.assertTrue(np.allclose(blade_matrix, standalone_matrix, rtol=1e-2, atol=1e-2),
    ...
    def test_boundary_submeshes(self):
        ...
        for i, (blade_frame_func, standalone_frame_func) in enumerate(zip(blade_left['frame'], standalone_left['frame'])):
            blade_frame_values = blade_frame_func.x.array
            standalone_frame_values = standalone_frame_func.x.array
            self.assertTrue(np.allclose(blade_frame_values, standalone_frame_values, rtol=1e-10, atol=1e-10),
        ...
        # Compare boundary frames
        for i, (blade_frame_func, standalone_frame_func) in enumerate(zip(blade_right['frame'], standalone_right['frame'])):
            blade_frame_values = blade_frame_func.x.array
            standalone_frame_values = standalone_frame_func.x.array
            self.assertTrue(np.allclose(blade_frame_values, standalone_frame_values, rtol=1e-10, atol=1e-10),
    ...
    def test_orientation_vectors(self):
        ...
        for i, (blade_frame_func, standalone_frame_func) in enumerate(zip(blade_segment.frame, standalone_segment.frame)):
            blade_frame_values = blade_frame_func.x.array
            standalone_frame_values = standalone_frame_func.x.array
            self.assertTrue(np.allclose(blade_frame_values, standalone_frame_values, rtol=1e-10, atol=1e-10),
