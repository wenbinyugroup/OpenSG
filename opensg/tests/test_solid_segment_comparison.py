...
class TestSolidSegmentComparison(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.segment_mesh_file = Path("..", "..", "examples", "data", "Solid OpenSG Beam", "bar_urc_npl_2_ar_10", "bar_urc_npl_2_ar_10-segment_0.yaml")
        
        if not Path(cls.segment_mesh_file).exists():
            raise FileNotFoundError(f"Solid segment mesh file not found: {cls.segment_mesh_file}")
        ...
        for i, es in enumerate(cls.blade_mesh.sets['element']):
            print(f"  Blade Set {i}: name='{es['name']}', labels[0]={es['labels'][0]}, num_labels={len(es['labels'])}")
        ...
        print(f"Standalone element sets: {len(cls.standalone_segment.sets['element'])} sets")
        for i, es in enumerate(cls.standalone_segment.sets['element']):
            print(f"  Standalone Set {i}: name='{es['name']}', labels[0]={es['labels'][0]}, num_labels={len(es['labels'])}")
        print(f"Standalone elLayID unique values: {np.unique(cls.standalone_segment.elLayID)}")
        ...
        if len(np.unique(cls.standalone_segment.elLayID)) == 1:
            print(f"NOTE: This appears to be a single-material segment (all layup IDs = {np.unique(cls.standalone_segment.elLayID)[0]})")
            print(f"This is expected if the segment YAML file contains only elements from one material")
        
        # Debug: Print original_cell_index information
        print(f"Blade segment original_cell_index shape: {cls.blade_segment.original_cell_index.shape}")
        print(f"Blade segment original_cell_index first 10: {cls.blade_segment.original_cell_index[:10]}")
        print(f"Standalone segment original_cell_index shape: {cls.standalone_segment.original_cell_index.shape}")
        print(f"Standalone segment original_cell_index first 10: {cls.standalone_segment.original_cell_index[:10]}")
    ...
    def test_segment_creation_methods(self):
        ...
        print(f"  Blade segment: {self.blade_segment.mesh.geometry.x.shape[0]} nodes, "
              f"{self.blade_segment.mesh.topology.index_map(3).size_local} elements")
        print(f"  Standalone segment: {self.standalone_segment.mesh.geometry.x.shape[0]} nodes, "
    ...
    def test_node_coordinates(self):
        ...
        # Check shapes
        self.assertEqual(blade_nodes.shape, standalone_nodes.shape, 
    ...
    def test_element_connectivity(self):
        ...
        standalone_elements = self.standalone_segment.mesh.topology.connectivity(3, 0).array
        
        # Check shapes
        self.assertEqual(blade_elements.shape, standalone_elements.shape,
    ...
    def test_material_database(self):
        ...
        # Check structure
        self.assertEqual(len(blade_materials), len(standalone_materials),
        ...
        self.assertEqual(len(blade_mat_params), len(standalone_mat_params),
                        "Should have the same number of materials")
        
        # Check each material parameter array
        for i, (blade_param, standalone_param) in enumerate(zip(blade_mat_params, standalone_mat_params)):
            self.assertTrue(np.allclose(blade_param, standalone_param, rtol=1e-10, atol=1e-10),
                          f"Material parameters {i} should match")
        
        # Check density values
        self.assertTrue(np.allclose(blade_density, standalone_density, rtol=1e-10, atol=1e-10),
    ...
    def test_orientation_vectors(self):
        ...
        for vector_name in vectors_to_check:
            ...
            self.assertTrue(np.allclose(blade_values, standalone_values, rtol=1e-10, atol=1e-10),
    ...
    def test_boundary_submeshes(self):
        ...
        for key in required_keys:
            ...
            self.assertIn(key, standalone_left, f"Standalone left submesh missing {key}")
        
        # Compare mesh geometries
        blade_left_nodes = blade_left['mesh'].geometry.x
        standalone_left_nodes = standalone_left['mesh'].geometry.x
        self.assertTrue(np.allclose(blade_left_nodes, standalone_left_nodes, rtol=1e-10, atol=1e-10),
        ...
        for key in required_keys:
            ...
            self.assertIn(key, standalone_right, f"Standalone right submesh missing {key}")
        
        blade_right_nodes = blade_right['mesh'].geometry.x
        standalone_right_nodes = standalone_right['mesh'].geometry.x
        self.assertTrue(np.allclose(blade_right_nodes, standalone_right_nodes, rtol=1e-10, atol=1e-10),
    ...
    def test_meshdata_structure(self):
        ...
        self.assertEqual(blade_keys, standalone_keys, "Meshdata should have the same keys")
    ...
    def test_layup_ids(self):
        ...
        self.assertEqual(blade_layup_ids.shape, standalone_layup_ids.shape,
                        "Layup ID arrays should have the same shape")
        
        # Debug: Print some information about the layup IDs
        print(f"Blade layup IDs: dtype={blade_layup_ids.dtype}, unique values={np.unique(blade_layup_ids)}")
        print(f"Standalone layup IDs: dtype={standalone_layup_ids.dtype}, unique values={np.unique(standalone_layup_ids)}")
        ...
        if len(np.unique(standalone_layup_ids)) == 1:
            print(f"NOTE: This is a single-material segment. Checking that all blade layup IDs match the single value.")
            ...
            self.assertTrue(layup_ids_match, f"All layup IDs should be {single_value} for single-material segment")
        else:
    ...
    def test_solid_workflow_information(self):
        ...
        self.assertGreater(len(self.blade_mesh.mat_name), 0, "Should have at least one material")
        
        # Verify material parameters
        self.assertEqual(len(blade_mat_params), len(self.blade_mesh.mat_name),
