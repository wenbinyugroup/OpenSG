...
mesh, subdomains, boundaries = gmshio.read_from_msh("3Dshell_tapered.msh", MPI.COMM_WORLD,0, gdim=3)
...
import pyvista
pyvista.start_xvfb()
u_topology, u_cell_types, u_geometry=dolfinx.plot.vtk_mesh(mesh_r,fdim)
...
def Cab(e,l,m):
    E1,E2,E3=e
    return as_tensor([(ddot(e1.vector[m],E1.vector[l]),ddot(e1.vector[m],E2.vector[l]),ddot(e1.vector[m],E3.vector[l])),
               (ddot(e2.vector[m],E1.vector[l]),ddot(e2.vector[m],E2.vector[l]),ddot(e2.vector[m],E3.vector[l])),
               (ddot(e3.vector[m],E1.vector[l]),ddot(e3.vector[m],E2.vector[l]),ddot(e3.vector[m],E3.vector[l]))])
...
for i,xx in enumerate(boundary_facets):
    ...
    for k in range(deg+1):
        if dofs[k] not in dof_S2L:
            ...
            print(e1l.vector[l],e1.vector[m],1)
            print(e2l.vector[l],e2.vector[m],2)
            print(e3l.vector[l],e3.vector[m],3) 
...
for i,xx in enumerate(boundary_facets):
    ...
    for k in range(deg+1):
        if dofs[k] not in dof_S2R:
            ...
            m=3*dofs[k],3*dofs[k]+1,3*dofs[k]+2
            e1r.vector[l]=e1.vector[m]
            e2r.vector[l]=e2.vector[m]
            e3r.vector[l]=e3.vector[m] 
    ...
    for i,xx in enumerate(boundary_facets):
        ...
        for k in range(deg+1):
            if dofs[k] not in dof_S2L:
                ...
                l=3*dofs_left[k],3*dofs_left[k]+1, 3*dofs_left[k]+2
                m=3*dofs[k],3*dofs[k]+1,3*dofs[k]+2
                v2a.vector[m]=Cab(e,l,m)*as_vector(w_l.vector[l])
    # Right
    e=[e1r,e2r,e3r]
    ...
    for i,xx in enumerate(boundary_facets):
        ...
        for k in range(deg+1):
            if dofs[k] not in dof_S2R:
                ...
                r=3*dofs_right[k],3*dofs_right[k]+1, 3*dofs_right[k]+2
                m=3*dofs[k],3*dofs[k]+1,3*dofs[k]+2
                v2a.vector[m]=Cab(e,r,m)*as_vector(w_r.vector[r])  
...
#------------------------------------------------------------------------------------- 
 ...
 #   problem = dolfinx_mpc.LinearProblem(a, b, mpc, petsc_options={"ksp_type": "preonly", "pc_type": "lu","pc_factor_mat_solver_type":"mumps"})
  #  problem.A.setNullSpace(nullspace)
  #  uu = problem.solve()

    V1s= scipy.sparse.linalg.spsolve(AA, FF, permc_spec=None, use_umfpack=True)
    Dhe[:,p]=  petsc.assemble_vector(b)
