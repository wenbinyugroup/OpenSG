...
# For printing boundary mesh:
# Replace with- u_topology, u_cell_types, u_geometry=dolfinx.plot.vtk_mesh(mesh_l,fdim)  
...
######## Define Functions for local shell frame
...
################### Truncation Error for Coefficient Matrix ################################
v2a=Function(V)
e=[e1,e2,e3]
    ...
    for i,xx in enumerate(boundary_facets):
        ...
        for k in range(deg+1):
            if dofs[k] not in dof_S2L:
                ...
                for kk in range(3):
                    v2a.vector[3*dofs[k]+kk] = w_l.vector[3*dofs_left[k]+kk] 
    ...
    for i,xx in enumerate(boundary_facets):
        ...
        for k in range(deg+1):
            if dofs[k] not in dof_S2R:
                ...
                for kk in range(3):
                    v2a.vector[3*dofs[k]+kk] = w_r.vector[3*dofs_right[k]+kk] 
    # ------------------------------------------------------------------------------
    # Solve 2D SG mesh using mapped boundary solutions under Dirichilet BC
    # ------------------------------------------------------------------------------
    w=Function(V)
    e=[e1,e2,e3]
    x, dx=SpatialCoordinate(mesh), Measure('dx')(domain=mesh)
...
#------------------------------------------------------------------------------------- 
 ...
 #   problem = dolfinx_mpc.LinearProblem(a, b, mpc, petsc_options={"ksp_type": "preonly", "pc_type": "lu","pc_factor_mat_solver_type":"mumps"})
  ...
  #  uu = problem.solve()
    ...
    V1s= scipy.sparse.linalg.spsolve(AA, FF, permc_spec=None, use_umfpack=True) # Sparse form iterative solver
    # Can use other way for fast computation
    Dhe[:,p]=  petsc.assemble_vector(b)  # Original Dhe matrix (without any dirichilet bc)
    V0[:,p]= V1s # Solved V0 matrix  
    print(p) 
